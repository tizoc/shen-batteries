...
`(seq.zip SeqA SeqB)` is equivalent to `(seq.zip-with (lambda A (lambda B (@p A B))) SeqA SeqB)`.

#### seq.unzip
**Type**: `(seq.t (A * B)) --> ((seq.t A) * (seq.t B))`

`(seq.unzip SeqA*B)` returns `(@p SeqA SeqB)`, where `SeqA*B` is a sequence of tuples `(@p A B)`, `SeqA` is `(seq.map (lambda V133 (fst V133)) SeqA*B)` and `SeqB` is `(seq.map (lambda V134 (snd V134)) SeqA*B)`.
